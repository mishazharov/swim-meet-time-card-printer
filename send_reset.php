<?php
	//$_POST['username'] should actually be their email
	...
	$stmt = $mysqli->prepare("SELECT id, setup FROM users WHERE deleted=0 AND email LIKE ?");
	...
	$stmt->close();
	$stmt = $mysqli->prepare("SELECT timestamp FROM reset_password WHERE user_id=? ORDER BY id DESC");
	$stmt->bind_param("i", $id);
	$stmt->execute();
	$stmt->bind_result($n_timestamp);
	$stmt->fetch();
